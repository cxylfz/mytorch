...
/**
 * Partner System Block Structure
 */
struct BuddyBlock {
  size_t size;    // Block Size
  size_t addr;    // Block Address
  void* ptr;   // Actual Memory Pointer
  
  BuddyBlock(size_t s, size_t a, void* p = nullptr) : size(s), addr(a), ptr(p) {}
  // Constructor for search
  BuddyBlock(size_t s) : size(s), addr(-1), ptr(nullptr) {}
};
...
struct HostBlock {
  ...
  HostBlock(size_t size, void* ptr, BuddyBlock buddy_block) 
    : size_(size), ptr_(ptr), buddy_block_(buddy_block) {}
  ...
  BuddyBlock buddy_block_{0, 0};
  bool from_buddy_system_{false}; //Mark whether it comes from the buddy system
};
...
class BuddySystemManager {
private:
  std::vector<std::vector<BuddyBlock>> free_lists_;
...
public:
  BuddySystemManager(size_t max_size = 1024 * 1024 * 1024) // Default 1GB
    ...
    free_lists_.resize(max_index + 1);
  }

  // Initialize buddy system
  bool initialize(void* base_address, size_t total_size) {
    ...
    base_ptr_ = base_address;
    max_block_size_ = c10::llvm::PowerOf2Floor(total_size);
    
    // Create initial maximum block
    size_t max_index = c10::llvm::Log2_64_Ceil(max_block_size_);
    if (free_lists_.size() <= max_index) {
      free_lists_.resize(max_index + 1);
    }
    
    BuddyBlock initial_block(max_block_size_, 0, base_ptr_);
    free_lists_[max_index].push_back(initial_block);
    
    return true;
  }

  // Allocate Memory
  std::pair<bool, BuddyBlock> allocate(size_t need_size) {
    std::lock_guard<std::mutex> lock(mutex_);
    
    if (base_ptr_ == nullptr) {
      return {false, BuddyBlock(0, 0)};
    }
    
    // Find the smallest power of two that satisfies the requirement
    size_t block_size = 1;
    while (block_size < need_size) {
      block_size *= 2;
    }
    ...
    while (current_size <= max_block_size_) {
      size_t index = c10::llvm::Log2_64_Ceil(current_size);
      if (index >= free_lists_.size()) {
        break;
      }
      
      if (!free_lists_[index].empty()) {
        BuddyBlock block = free_lists_[index].back();
        free_lists_[index].pop_back();
        
        // If the found block is larger than needed, split it.
        while (current_size > block_size) {
          current_size /= 2;
          size_t new_index = c10::llvm::Log2_64_Ceil(current_size);
          
          // Create two buddy blocks.
          BuddyBlock buddy1(current_size, block.addr, 
                           static_cast<uint8_t*>(base_ptr_) + block.addr);
          BuddyBlock buddy2(current_size, block.addr + current_size,
                           static_cast<uint8_t*>(base_ptr_) + block.addr + current_size);
          
          free_lists_[new_index].push_back(buddy2);
          block = buddy1;
        }
  ...
  // Free Memory
  void free_block(const BuddyBlock& block) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    size_t current_addr = block.addr;
    BuddyBlock current_block = block;
    
    while (block_size < max_block_size_) {
      size_t index = c10::llvm::Log2_64_Ceil(block_size);
      bool merged = false;
      
      // Find the buddy block.
      for (auto it = free_lists_[index].begin(); it != free_lists_[index].end(); ++it) {
        
        if ((current_addr + block_size == it->addr) || 
            (current_addr - block_size == it->addr)) {
          
          if ((current_addr ^ it->addr) == block_size) {
            
            size_t new_addr = std::min(current_addr, it->addr);
            free_lists_[index].erase(it);
            block_size *= 2;
            current_addr = new_addr;
            current_block = BuddyBlock(block_size, new_addr, 
                                     static_cast<uint8_t*>(base_ptr_) + new_addr);
            merged = true;
            break;
          }
        }
    ...
    size_t final_index = c10::llvm::Log2_64_Ceil(block_size);
    if (final_index < free_lists_.size()) {
      free_lists_[final_index].push_back(current_block);
    }
  }
...
struct CachingHostAllocatorImpl {
  ...
  // Initialize buddy system.
  virtual bool initialize_buddy_system(size_t pool_size = 0) {
    if (buddy_initialized_) {
      return true;
    }
    
    if (pool_size > 0) {
      buddy_pool_size_ = pool_size;
    }
    
    // Allocate large memory block for buddy system.
    void* buddy_pool = nullptr;
    allocate_host_memory(buddy_pool_size_, &buddy_pool);
    
    if (!buddy_pool) {
      return false;
    }
    
    buddy_system_ = std::make_unique<BuddySystemManager>(buddy_pool_size_);
    if (buddy_system_->initialize(buddy_pool, buddy_pool_size_)) {
      buddy_initialized_ = true;
      return true;
    }
 ...
 public:
  // Modify the allocate method to prioritize using the buddy system.
  virtual std::pair<void*, void*> allocate(size_t size) {
    if (size == 0) {
      return {nullptr, nullptr};
    }

    // Lazy initialization of the buddy system.
    if (!buddy_initialized_ && size <= buddy_pool_size_) {
    ...
    // First try to allocate from the buddy system.
    if (buddy_initialized_ && roundSize <= buddy_pool_size_) {
      auto [success, buddy_block] = buddy_system_->allocate(roundSize);
      if (success) {
        B* block = new B(roundSize, buddy_block.ptr, buddy_block);
        block->allocated_ = true;
        block->from_buddy_system_ = true;
  ...
  // Modify the free method to support buddy system memory recycling.
  virtual void free(void* ctx) {
    ...
    // If the memory is allocated by the buddy system, handle it specially.
    if (block->from_buddy_system_ && buddy_initialized_) {
      buddy_system_->free_block(block->buddy_block_);
    ...
    // Original logic handles non-buddy system memory.
  ...
  // Modify the empty_cache method to not clear buddy system memory.
  virtual void empty_cache() {
    process_events();

    // Only clear traditional cache, preserve buddy system memory.
    for (size_t i = 0; i < free_list_.size(); ++i) {
      ...
      for (auto* block : free_list_[i].list_) {
        // Skip memory allocated by the buddy system.
        if (!block->from_buddy_system_) {
          blocks_to_remove.push_back(block);
        }
      }

      for (auto* block : blocks_to_remove) {
        free_list_[i].list_.erase(
            std::remove(free_list_[i].list_.begin(), free_list_[i].list_.end(), block),
            free_list_[i].list_.end());
        blocks_.erase(block);
        ptr_to_block_.erase(block->ptr_);
        free_block(block);
        stats_.allocations.decrease(1);
        stats_.allocated_bytes.decrease(block->size_);
        stats_.allocation_bucket_stats[i].decrease(1);
        stats_.allocated_bytes_bucket_stats[i].decrease(block->size_);
        delete block;
      }
    }
  }

  // Add buddy system configuration method.
  virtual void set_buddy_pool_size(size_t size) {
    if (!buddy_initialized_) {
      buddy_pool_size_ = size;
    }
  }
